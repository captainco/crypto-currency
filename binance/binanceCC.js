...
async function Main() {
    ...
    UpdateUSDT.on('message', async (event) => {
        try {
            process.env.Webhooku = Number(totalUSDT).toFixed(2);
        } catch (e) {
    ...
    Trading.on('message', async (event) => {
        try {
            if (
                    process.env.Webhook1m == process.env.Webhook5m
                    && process.env.Webhook1m == process.env.Webhook15m
                    && process.env.Webhook1m == process.env.Webhook30m
                    && process.env.Webhook1m == process.env.Webhook1h
                    && process.env.Webhook1m != checkTrend
                )
            {
