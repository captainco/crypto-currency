...
async function Main() {
    ...
    UpdateUSDT.on('message', async (event) => {
        try {
            process.env.Webhook30mu = Number(totalUSDT).toFixed(2);
        } catch (e) {
    ...
    Trading.on('message', async (event) => {
        try {
            if (process.env.Webhook30m == '' || process.env.Webhook30m == checkTrend) {
                return;
            }
            checkTrend = process.env.Webhook30m;
            longShortCond = process.env.Webhook30m == 'buy' ? 'LONG' : 'SHORT';
            const Ps = (await binance.FuturesPositionRisk('BTCUSDT'))[0];
            if (longShortCond == 'LONG') {
                if (isTrade == 0) {
                    isTrade = 1;
                    await telegram.log(`🟢BTCUSDT 30m. E: ${Number(Ps.markPrice).toFixed(2)}; T: ${Number(totalUSDT).toFixed(2)} USDT`);
                    ...
                    if (isTrade == -1) {
                        ...
                        await telegram.log(`${iconLongShortAlert}🔴BTCUSDT 30m. E: ${Number(markPricePre).toFixed(2)}; M: ${Number(Ps.markPrice).toFixed(2)}; TPSL: ${tpslUSDT} USDT; T: ${Number(totalUSDT).toFixed(2)} USDT`);
                        await telegram.log(`🟢BTCUSDT 30m. E: ${Number(Ps.markPrice).toFixed(2)}; T: ${Number(totalUSDT).toFixed(2)} USDT`);
                        markPricePre = Number(Ps.markPrice);
                    }
                }
